...
import { deleteReview } from "~/services/review.server";
...
export default function DeleteReview() {
    ...
    return (
        <Modal onClose={closeHandler} className="w-11/12 sm:w-2/5">
            <h1 className="text-xl font-bold mb-2">Delete Review!</h1>
            <p className="text-sm text-gray-500 my-2">This action cannot be undone. This will permanently delete:<br /> <span className="font-bold text-primary mt-4">" {id} "</span></p>
...
export async function action({ params, request }: ActionFunctionArgs) {
    ...
    if (!roleId) throw new Response("Review ID is required", { status: 400 });
    if (request.method === "DELETE") {
        try {
            const res = await deleteReview(roleId, userId);
            if (res.id) {
                return redirect("/dashboard/reviews?success=Delete+review+successfully");
            }
        } catch (error: any) {
