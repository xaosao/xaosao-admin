...
// backend 
import { deleteRole } from "~/services/role.server";
...
export default function DeleteRoles() {
    ...
    return (
        <Modal onClose={closeHandler} className="w-11/12 sm:w-2/5">
            <h1 className="text-xl font-bold mb-2">Delete Roles!</h1>
                ...
                <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                    <div className="flex items-start space-x-2">
                        ...
                        <div className="text-sm text-red-800 flex items-start flex-col gap-1">
                            ...
                            <p>All customer data, chat history, and transaction records will be permanently deleted.</p>
                        </div>
                    </div>
                </div>
                <div className="mt-2">
...
export async function action({ params, request }: ActionFunctionArgs) {
    ...
    if (!roleId) throw new Response("Role ID is required", { status: 400 });
    if (request.method === "DELETE") {
        try {
            const res = await deleteRole(roleId, userId);
            if (res.id) {
                return redirect("/dashboard/roles?success=Delete+role+successfully");
            }
        } catch (error: any) {
