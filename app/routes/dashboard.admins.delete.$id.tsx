...
export default function DeleteAdmin() {
    ...
    return (
        <Modal onClose={closeHandler} className="w-11/12 sm:w-2/5">
            <h1 className="text-xl font-bold mb-2">Delete Admin!</h1>
            <p className="text-sm text-gray-500 my-4">This action cannot be undone. This will permanently delete:<br /><span className="font-bold text-primary mt-2">" {id} "</span></p>
            <Form method="delete" className="space-y-4 mt-4">
                <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                    <div className="flex items-start space-x-2">
                        <AlertTriangle className="h-4 w-4 text-orange-600 mt-0.5" />
                        <div className="text-sm text-gray-800 flex items-start flex-col gap-1">
                            <p className="font-medium">This action is irreversible!</p>
                            <p>All admin data, chat history, and transaction records will be permanently deleted.</p>
                        </div>
                    </div>
                </div>
                <div>
...
export async function action({ params, request }: ActionFunctionArgs) {
    ...
    if (!roleId) throw new Response("Admin ID is required", { status: 400 });
    if (request.method === "DELETE") {
        try {
            const res = await deleteAdmin(roleId, userId);
            if (res.id) {
                return redirect("/dashboard/admins?success=Delete+admin+successfully");
            }
        } catch (error: any) {
            console.log("ENABLE_2FA_ERROR", error);
