import { AlertTriangle, UserCheck, Mars, Venus, X } from "lucide-react"
...
import { getModel, rejectModel } from "~/services/model.server"
import { requireUserPermission, requireUserSession } from "~/services/auth.server"

export default function RejectModelModal() {
    ...
    return (
        <Modal onClose={closeHandler} className="w-11/12 sm:w-2/5">
            <div className="space-y-6 py-4 sm:py-2">
                <div>
                    <h3 className="flex items-center text-black text-md font-bold text-red-500"> <X className="h-5 w-5" />&nbsp;Reject Model</h3>
                    <p className="text-gray-500 text-sm ml-4 mt-2"> Confirm reject model. This action will process the payment immediately.</p>
                </div>
                <div className="space-y-4">
                    <Card>
                        <CardContent className="p-4 rounded border border-red-500 bg-red-50">
                    ...
                    <Card className="border-orange-200 bg-orange-50">
                        <CardContent className="p-4">
                            <div className="flex items-start space-x-3">
                                ...
                                <div>
                                    <h4 className="text-sm font-semibold text-orange-800">Approval Impact</h4>
...
export async function action({ params, request }: ActionFunctionArgs) {
    ...
    if (request.method === "PATCH") {
        try {
            ...
            console.log("REJECT_MODEL_FAILED", error);
