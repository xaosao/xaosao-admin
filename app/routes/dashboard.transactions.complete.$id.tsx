...
import { getTransaction, completeHeldTransaction } from "~/services/transaction.server"
    ...
    return (
        <Modal onClose={closeHandler} className="w-11/12 sm:w-2/5 p-4">
            <div className="space-y-4">
                <div>
                    <h3 className="flex items-center text-black text-md font-bold text-green-500"> <CheckCircle className="h-5 w-5" />&nbsp;Complete Transaction</h3>
                    <p className="text-gray-500 text-sm ml-4 mt-2">Release payment to the model with commission deduction.</p>
                </div>
                <div className="space-y-4">
                    {/* Model Info */}
                    <Card>
                        <CardContent className="p-4 rounded border border-green-500 bg-green-50">
                            <div className="space-y-2">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <div>
                                        <p className="text-sm text-gray-600">{model?.firstName ?? "Unknown"}&nbsp;{model?.lastName ?? ""}</p>
...
export async function action({ params, request }: ActionFunctionArgs) {
    ...
    if (request.method === "PATCH") {
        try {
            ...
            console.log("COMPLETE_TRANSACTION_FAILED", error);
