...
import { deleteModel } from "~/services/model.server";
...
export default function DeleteModel() {
    ...
    return (
        <Modal onClose={closeHandler} className="w-11/12 sm:w-2/5">
            <h1 className="text-xl font-bold mb-2">Delete Model!</h1>
            ...
            <Form method="delete" className="space-y-4 mt-4">
                <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                    <div className="flex items-start space-x-2">
                        ...
                        <div className="text-xs text-red-800 flex items-start flex-col gap-1">
                            <p className="font-medium">This action is irreversible!</p>
                            <p>All model data, chat history, and transaction records will be permanently deleted.</p>
                        </div>
                    </div>
                </div>
                <div>
...
export async function action({ params, request }: ActionFunctionArgs) {
    ...
    if (!modelId) throw new Response("Model ID is required", { status: 400 });
    if (request.method === "DELETE") {
        try {
            const res = await deleteModel(modelId, userId);
            if (res.id) {
                return redirect("/dashboard/models?success=Delete+model+successfully");
            }
        } catch (error: any) {
