...
import { deleteService } from "~/services/service.server";
...
export default function DeleteServices() {
    ...
    return (
        <Modal onClose={closeHandler} className="w-11/12 sm:w-2/4">
            <h1 className="text-xl font-bold">Delete Services!</h1>
                ...
                <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                    <div className="flex items-start space-x-2">
                        ...
                        <div className="text-sm text-red-800">
                            ...
                            <p>All services data, chat history, and transaction records will be permanently deleted.</p>
                        </div>
                    </div>
                </div>
                <div>
...
export async function action({ params, request }: ActionFunctionArgs) {
    ...
    if (!serviceId || !userId) throw new Response("Service ID is required");

    if (request.method === "DELETE") {
        try {
            const res = await deleteService(serviceId, userId);
            if (res.id) {
                return redirect("/dashboard/services?success=Delete+service+successfully");
            }
        } catch (error: any) {
